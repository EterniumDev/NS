...
LINK_ENTITY_TO_CLASS(kwPistolB, AvHPistolB);
void V_PunchAxis(int axis, float punch);



void AvHPistolB::Init()
{
	this->mRange = kHGRange;
	this->mDamage = BALANCE_VAR(kHGDamage);
}

int	AvHPistolB::GetBarrelLength() const
{
	return kHGBarrelLength;
}

float AvHPistolB::GetRateOfFire() const
{
	return BALANCE_VAR(kHGROF);
}

int	AvHPistolB::GetDeployAnimation() const
{
	return 6;
}

char* AvHPistolB::GetDeploySound() const
{
	return kTECDeploySound;
}

float AvHPistolB::GetDeployTime() const
{
	return .35f;
}

int	AvHPistolB::GetEmptyShootAnimation() const
{
	return 5;
}

char* AvHPistolB::GetHeavyViewModel() const
{
	return kTECHVVModel;
}

char* AvHPistolB::GetPlayerModel() const
{
	return kTECPModel;
}

Vector AvHPistolB::GetProjectileSpread() const
{
	return VECTOR_CONE_20DEGREES;
}

char* AvHPistolB::GetViewModel() const
{
	return kTECVModel;
}

char* AvHPistolB::GetWorldModel() const
{
	return kTECWModel;
}

int	AvHPistolB::GetReloadAnimation() const
{
...
int	AvHPistolB::GetShootAnimation() const
{
	return 4;
}

bool AvHPistolB::GetHasMuzzleFlash() const
{
	return true;
}

bool AvHPistolB::GetMustPressTriggerForEachShot() const
{
	return true;
}

float AvHPistolB::GetReloadTime(void) const
{
	return 3.0f;
}

void AvHPistolB::Precache()
{
	...
	this->mEvent = PRECACHE_EVENT(1, kTECEventName);
}


void AvHPistolB::Spawn()
{
	AvHMarineWeapon::Spawn();

	Precache();

	this->m_iId = AVH_WEAPON_PISTOLB;
	this->m_iDefaultAmmo = BALANCE_VAR(kHGMaxClip)*(BALANCE_VAR(kMarineSpawnClips) + 1);

	// Set our class name
	this->pev->classname = MAKE_STRING(kwsPistolB);

	SET_MODEL(ENT(this->pev), kTECWModel);
