...
BOOL CHalfLifeMultiplay :: GetNextBestWeapon( CBasePlayer *pPlayer, CBasePlayerItem *pCurrentWeapon )
{
	...
	if ( pCurrentWeapon != NULL && !pCurrentWeapon->CanHolster() )
	{
	...
	for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		...
		while ( pCheck )
		{
			if ( pCurrentWeapon != NULL && pCheck->iWeight() > -1 && pCheck->iWeight() == pCurrentWeapon->iWeight() && pCheck != pCurrentWeapon )
			{
				// this weapon is from the same category. 
				if ( pCheck->CanDeploy() && pCheck->IsUseable())
				{
...
edict_t *CHalfLifeMultiplay::GetPlayerSpawnSpot( CBasePlayer *pPlayer )
{
	...
	if ( IsMultiplayer() && !FNullEnt(pentSpawnSpot) && (pentSpawnSpot->v.target))
	{
...
BOOL CHalfLifeMultiplay :: PlayFootstepSounds( CBasePlayer *pl, float fvol )
{
	...
	if ( pl->IsOnLadder() || pl->pev->velocity.Length2D() > pl->GetMaxWalkSpeed() )
...
void GetMapNamesFromMapCycle(StringList& outMapNameList)
{
    ...
    if ( pFileList && length )
    {
        ...
        while ( 1 )
        {
            ...
            {
                outMapNameList.push_back( szMap );
            }
        }
